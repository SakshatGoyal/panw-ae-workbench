...
// Intentional inline SVG (NOT a stale lucide / hand-rolled fallback).
//
// The radio glyph carries TWO distinct colors at once — the ring uses the
// neutral icons.secondary family and the dot uses the brand family. The
// `@ds/icons` `RadioButton{Filled,Empty}` icons are monochrome (every path
// paints with currentColor), so they cannot satisfy this bi-color
// requirement in a single element.
//
// Decision: keep two paths in one local SVG so the ring and dot can be
// styled independently via `.panw--rb-ring` / `.panw--rb-dot`. The
// geometry mirrors the DS icon shapes; only the coloring strategy differs.
const RING_PATH =
