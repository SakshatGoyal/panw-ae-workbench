...
// No icon imports — AiPromptIcon is defined inline below
import './suggestions.css'

// ─── Icons ────────────────────────────────────────────────────────────────────

/**
 * ai-icon-prompt — inline SVG component derived from .tmp/ai-icon-prompt.svg.
 * Paths use fill="currentColor" so the icon inherits DS color tokens via CSS.
 */
...
const DEFAULT_SUGGESTIONS: SuggestionItem[] = [
  ...
  {
    id: '6',
    label: 'Which of my pipeline deals are stalled at technical evaluation?',
  },
  {
    id: '7',
    label: 'Summarise the top risk findings from my last Cortex XDR incident',
  },
]
...
// icon is on the right (iconPosition="right"), so space-between pushes it
// to the trailing edge while text remains flush left.
      ...
      <div className="sg-canvas">
        <div className="sg-demo">
          <div className="sg-demo__group">
            <div className="sg-demo__label">Suggestions</div>
            <Suggestions
              onSelect={(id) => {
